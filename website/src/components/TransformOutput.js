...
function transform(transformer, transformCode, code) {
  ...
  return transformer._promise.then(([realTransformer, toES5]) => {
    ...
    if (realTransformer.nonJavascript) {
      result = transformer.transform(realTransformer, transformCode, code);
    } else {
      ...
      result = transformer.transform(realTransformer, es5Code, code);
    }
...
export default class TransformOutput extends React.Component {
  ...
  componentDidMount() {
    transform(this.props.transformer, this.props.transformCode, this.props.code).then(
  ...
  componentWillReceiveProps(nextProps) {
    if (
      ...
      transform(nextProps.transformer, nextProps.transformCode, nextProps.code)
        .then(
